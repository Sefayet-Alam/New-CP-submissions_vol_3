...
int main()
{
    ...
    while(t--){
        ...
        for(ll i=0;i<n;i++){
            ...
            vector<ll>pos;
            for(ll j=0;j<m;j++){
                if(grid[i][j]!=vec[j]){cnt++;pos.push_back(j);}
            }
            if(cnt==0){
                continue;
            }
        ...
        if(f==2){
            for(ll i=0;i<n;i++){
                vector<ll>vec;
                for(ll j=0;j<m;j++){vec.push_back(grid[i][j]);}
                swap(vec[change.first],vec[change.second]);
                if(!is_sorted(all(vec))){f=1;break;}
            }
        }
        if(f==1) cout<<-1<<nn;
        else{
            if(f==0) cout<<1<<" "<<1<<nn;
            else cout<<change.first+1<<" "<<change.second+1<<nn;
        }
