...
int main()
{
	...
	while(T--)
	{
		cin>>n>>x>>y;
		cin>>a>>b;tot=ans=0;
		for(ll i=0;i<n;i++)if(a[i]!=b[i])p[++tot]=i;
		if(tot&1){cout<<"-1\n";continue;}
		if(y<=x)
		{
		...
		for(ll len=2;len<=tot;len+=2)
		for(ll i=1;i<=tot;i++)
		{
			ll j=i+len-1;
			f[i][j]=f[i+1][j-1]+y;
			f[i][j]=min(f[i][j],f[i][j-2]+(p[j]-p[j-1])*x);
			f[i][j]=min(f[i][j],f[i+2][j]+(p[i+1]-p[i])*x);
		}
		cout<<f[1][tot]<<'\n';
	}
}
