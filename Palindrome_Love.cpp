...
int main()
{
    ...
    while (t--)
    {
      ...
      if(n==1){
        ...
        if(s[0]==s[1]) cout<<0<<nn;
        else cout<<-1<<nn;
      }
      else{
        PolyHash ps(s);
        if(ps.get_hash(0,n-1)==ps.rev_hash(0,n-1)) cout<<0<<nn;
        else if(ps.get_hash(1,n-1)!=ps.rev_hash(1,n-1) || ps.get_hash(0,n-2)!=ps.rev_hash(0,n-2)) cout<<1<<nn;
        else cout<<-1<<nn;
      }
    }
