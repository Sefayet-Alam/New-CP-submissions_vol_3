...
vector<ll>primenos;
vector<ll>favbyall(N,1);
vector<ll>favbyprim(N,0);
...
int main()
{
    ...
    for(ll i=2;i<N;i++){
        if((favbyall[i]%2)==(favbyprim[i]%2)){
            answers[i]++;
        }
    }
    // for(ll i=0;i<15;i++){
    //     cout<<i<<" "<<favbyall[i]<<" "<<favbyprim[i]<<" "<<answers[i]<<nn;
    // }
    for(ll i=1;i<N;i++){
    ...
    while(t--){
        ...
        // cout<<answers[r]<<" "<<answers[l-1]<<nn;
        ll ans=answers[r]-answers[l-1];
        cout<<ans<<nn;
