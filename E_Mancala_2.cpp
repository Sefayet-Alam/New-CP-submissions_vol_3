...
int main()
{
    ...
    while (t--)
    {
        ...
        for (ll i = 0; i < m; i++)
        {
            ...
            ll curr=queryLazySegTree(0,0,n-1,b[i],b[i]);
            updateLazySegTree(0, 0, n - 1, b[i], b[i], -curr);
            ...
            if(curr%n==0){    
                ll toadd = curr / n;          
                updateLazySegTree(0,0,n-1,0,n-1,toadd);
            }
            else if (curr > n)
            {  
                ll toadd = curr / n;
                ...
                if (st <= en)
                {
                updateLazySegTree(0, 0, n - 1, st, en, 1);
                }
                else
                {
                updateLazySegTree(0,0,n-1,st,n-1,1);
                updateLazySegTree(0,0,n-1,0,en,1);
                }
                }
            }
            else{
            ...
            // for (ll j = 0; j < n; j++)
            // {
            // ll ans =queryLazySegTree(0,0,n-1,j,j);
            // cout << ans << " ";
            // }
            // cout << nn;
