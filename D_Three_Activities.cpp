...
vector<ll>a,b,c;
ll dp[N][2][2][2];
ll func(ll i,bool a1,bool b1,bool c1){
    if(a1 && b1 && c1) return 0;
    if(i==n) return -LLONG_MAX;
    if(dp[i][a1][b1][c1]!=-1) return dp[i][a1][b1][c1];
    ll ans=0;
    ans=max(ans,func(i+1,a1,b1,c1));
    if(a1==0) ans=max(ans,a[i]+func(i+1,1,b1,c1));
     if(b1==0) ans=max(ans,b[i]+func(i+1,a1,1,c1));
      if(c1==0) ans=max(ans,c[i]+func(i+1,a1,b1,1));
