...
int main()
{
    ...
    while(t--){
        ...
        if(f){cout<<0<<nn;continue;}
    
        n-=x;
        if(op<0 || n<0 || 2*n+op<n){
            cout<<0<<nn;
        }
        else{
            //catalan convolution: C_n^(k) where there are k open brackets in the prefix
            //C_n^k=(k+1)/(n+k+1) * (2*n+k)C(n)
            ll ans=nCrMod(2*n+op,n,M);
            ans=(ans*(op+1))%M;
            ans=(ans*(inverseMod(n+op+1,M)))%M;
            ans=ans%M;
