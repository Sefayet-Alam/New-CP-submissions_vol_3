...
ll n,m,k;
const ll INFF = 3e18 + 5;
struct Matrix
{
    vector<vector<ll>> a = vector<vector<ll>>(n, vector<ll>(n, INFF));
...
int main()
{
    ...
    while (t--)
    {
        ...
        Matrix tot=expo_power(mat,k);
        ll ans=INFF;
        for(ll i=0;i<n;i++){
            for(ll j=0;j<n;j++){
                ans=min(ans,tot.a[i][j]);
            }
        }
        if(ans>INFF/2) cout<<"IMPOSSIBLE"<<nn;
        else cout<<ans<<nn;
    }
