...
int main()
{
    ...
    while(t--){
        ...
        vector<ll>ans(n);
        bool f=0;
        ans[n-1]=min(vec[n-1].second,vec[n-1].first);
        for(ll i=n-2;i>=0;i--){
            if(vec[i].second>=ans[i+1] &&  vec[i].first>=ans[i+1] ){
                   ans[i]=min(vec[i].second,vec[i].first);
            }
            else if(vec[i].second>=ans[i+1] ){
                ans[i]=vec[i].second;
            }
            else if(vec[i].first>=ans[i+1] ){
                ans[i]=vec[i].first;
            }
            else{
                f=1;
