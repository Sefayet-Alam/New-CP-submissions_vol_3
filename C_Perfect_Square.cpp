    ...
    while(t--){
      ...
      for(ll i=0;i<n/2;i++){
        ...
        for(ll j=i;j<n-i-1;j++){
            temp+=max({grid[i][j],grid[j][n-1-i],grid[n-1-i][n-1-j],grid[n-1-j][i]});
           
            ll maxm=max({grid[i][j],grid[j][n-1-i],grid[n-1-i][n-1-j],grid[n-1-j][i]});
           
            ll ex=(maxm-grid[i][j])+(maxm-grid[j][n-1-i])+(maxm-grid[n-1-i][n-1-j])+(maxm-grid[n-1-j][i]);
            curr+=ex;
