...
int main()
{
    ...
    while(t--){
        ...
        vector<ll>ansmin(k,LLONG_MAX),ansmax(k,LLONG_MIN);
        ll r=0;
        for(ll i=k-1;i>=0;i--){
            ansmax[i]=maxq.top();
            w[i]--;
            if(w[i]==0) ansmin[i]=maxq.top();
            maxq.pop();
        }
        while(maxq.size()){
            minq.push(maxq.top());
        ...
        while(minq.size()){
            while(w[r]){
                ll topp=minq.top();
                ansmin[r]=min(ansmin[r],topp);
                ansmax[r]=max(ansmax[r],topp);
                w[r]--;
                minq.pop();
            }
            r++;
        ...
        ll ans=0;
        for(ll i=0;i<k;i++){
            // cout<<ansmin[i]<<" "<<ansmax[i]<<nn;
            ans+=ansmin[i]+ansmax[i];
        }
        cout<<ans<<nn;
    }
