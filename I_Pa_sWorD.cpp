...
int main()
{
    ...
    while(t--){
      ...
      string s;
      cin>>s;
      ll wh=0,sm=0,big=0,dig=0;
      ll cumu=1;
      ll tot=1;
      ll tomin=0;
      for(ll i=0;i<n;i++){
        if(s[i]>='a' && s[i]<='z'){sm++;tot=(tot*2)%M;}
      ...
      for(ll i=1;i<n;i++){
        if(s[i]==s[i-1]){
            if(s[i]=='?'){
                tomin=(tomin+62*(wh-2)*62*sm*2)%M;
            }
            else if(s[i]>='a' && s[i]<='z'){
                tomin=(tomin+2*(wh)*62*(sm-2)*2)%M;
            }
        }
        else if((s[i]=='?' && s[i-1]>='a' && s[i-1]<='z')|| (s[i-1]=='?' && s[i]>='a' && s[i]<='z')){
             tomin=(tomin+2*(wh-1)*62*(sm-1)*2)%M;
        }
      }
