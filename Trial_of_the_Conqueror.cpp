...
ll dfs(int vertex,int par=-1){
    ...
    if(g[vertex].size()==3) path.push_back(vertex);
    ll ans=0;
    for(int child: g[vertex]){
       
        if(child==par) continue;
        ans=max(ans,dfs(child,vertex));
        
    }
    if(g[vertex].size()==3) path.pop_back();
    return ans;
}
...
int main()
{
    ...
    while(t--){
      ...
      for(ll i=1;i<=n;i++){
        if(g[i].size()==1){
            leaves.push_back({level[i],i});
        }
      }
      sort(all(leaves));
      reverse(all(leaves));
      vis2[1]=1;
      ll curr=leaves[0].second;
      while(par[curr]!=-1){
        vis2[curr]=1;
        curr=par[curr];
      }
      for(ll i=1;i<leaves.size();i++){
        ll curr_v=leaves[i].second;
      }
    }
