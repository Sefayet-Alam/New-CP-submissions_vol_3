...
bool checker(ll n,ll **grid){
    map<ll,ll>freq;
    for(ll i=1;i<=n;i++){
        for(ll j=1;j<=n;j++){
            if(GCD(grid[i][j],i)!=1 || GCD(grid[i][j],j)!=1) return false;
            freq[grid[i][j]]++;
        }
    }
    for(auto it:freq){
...
int main()
{
    ...
    while(t--){
      ...
      ll k=n*n;
      auto pos=upper_bound(all(primenos),k);
      pos--;
    //   deb(*pos);
      ll main=*pos;
      ll ex=k-*pos;
