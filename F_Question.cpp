...
int main()
{
    ...
    while(t--){
      ...
      for(ll i=0;i<n;i++){
        os.insert(vec[i]);
        // cout<<vec[i]<<" "<<os.order_of_key(vec[i])<<nn;
        if(os.order_of_key(vec[i])==os.size()-1){
            ans.push_back(vec[i]);
        }
      }
      os.clear();
      ...
      vector<ll>finl;
      for(ll i=0;i<n;i++){
        if(vis[vec[i]]==2) finl.push_back(vec[i]);
      }
        cout<<finl.size()<<" ";
        if(finl.size()){
            if(finl.size()>100){
                for(ll i=0;i<100;i++) cout<<finl[i]<<" ";
                cout<<nn;
            }
            else cout<<finl<<nn;
        }
