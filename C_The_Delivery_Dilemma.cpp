...
int main()
{
    ...
    while(t--){
      ...
      sort(all(vec));
      vector<ll>suff(n,0);
      suff[n-1]=vec[n-1].second;
      for(ll i=n-2;i>=0;i--){
        suff[i]+=vec[i].second+suff[i+1];
      }
      ll currmax=vec[0].first;
      ll maxm=*max_element(all(a));
      ll ans=min(suff[0],maxm);
      for(ll i=0;i<n-1;i++){
        currmax=max(currmax,vec[i].first);
        ans=min(ans,max(suff[i+1],currmax));
      }
      cout<<ans<<nn;
    }
