...
int main()
{
    ...
    while(t--){
      ...
      bool f=0;
      for(ll i=0;i<n;i++){
       if(ranges[i].first>currmax || ranges[i].second<currmin){
        f=1;
        break;
       }
       if(i==n-1) break;
       currt=times[i+1]-times[i];
       currmin=max(currmin,ranges[i].first)-currt;
       currmax=min(currmax,ranges[i].second)+currt;
      }
      if(f) cout<<"NO"<<nn;
      else cout<<"YES"<<nn;
    }
