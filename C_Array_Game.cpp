...
int main()
{
    ...
    while(t--){
      ...
      set<ll>diffs;
      for(ll i=0;i<n-1;i++){
        ll currdiff=abs(vec[i+1]-vec[i]);
        minm=min(currdiff,minm);
        diffs.insert(currdiff);
      }
      if(k==1){
        cout<<minm<<nn;
        continue;
    ...
    //   }
      for(ll i=0;i<n;i++){
        auto pos=diffs.lower_bound(vec[i]);
        if(pos!=diffs.end()){
           ll currdiff=abs(*pos-vec[i]);
           minm=min(minm,currdiff);
        }
      }
      for(ll i=0;i<n;i++){
        auto pos=diffs.upper_bound(vec[i]);
        if(pos!=diffs.end()){
           ll currdiff=abs(*pos-vec[i]);
           minm=min(minm,currdiff);
        }
