...
int main()
{
    ...
    while(t--){
        ...
        unordered_map<ll,ll> sufff,preff;
        for(ll i=0;i<n;i++){
            cin>>s;
            vec.push_back(s);
            ll sz=s.size();
        ...
        string str;
        for(ll i=0;i<n;i++){
            ll currsz=vec[i].size();
            ans+=totsz+n*currsz;
            PolyHash pl2(vec[i]);
            ll val=pl2.suff[1];
            if(preff.find(val)!=preff.end()) ans-=2*preff[val]*currsz;
            ll val2;
            for(ll j=2;j<=currsz;j++){
                val=pl2.suff[j-1];
                val2=pl2.suff[j];
                if(preff.find(val2)!=preff.end()) ans=ans-(preff[val2])*(currsz-j+1)*2;
                if(preff.find(val)!=preff.end()) ans=ans+(preff[val])*(currsz-j+1)*2;
            }
        }
        cout<<ans<<nn;
