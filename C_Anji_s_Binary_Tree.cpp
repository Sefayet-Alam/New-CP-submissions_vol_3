...
ll dp[N];
ll dfs(ll vertex){
    if(g[vertex][0]==g[vertex][1] && g[vertex][0]==0){
        // ans=min(ans,cnt);
        return dp[vertex]=0;
    }
    ...
    ll cnt=LLONG_MAX;
    //  cout<<vertex<<" "<<g[vertex]<<" "<<s[vertex]<<nn;
    if(g[vertex][0]){
    if(s[vertex]=='L'){
        cnt=min(cnt,dfs(g[vertex][0]));
    }
    else{
        cnt=min(cnt,1+dfs(g[vertex][0]));
    }
    }
    if(g[vertex][1]){
    if(s[vertex]=='R'){
        cnt=min(cnt,dfs(g[vertex][1]));   
    }
    else{
        cnt=min(cnt,1+dfs(g[vertex][1]));
    }
    }
