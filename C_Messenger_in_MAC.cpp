...
ll dpfunc(ll i,ll k){
    ...
    ll ans=LLONG_MAX;
    ans=min(ans,dpfunc(i+1,k));
    for(ll j=i+1;j<n;j++){
        if(k>1) ans=min(ans,vec[i].second+abs(vec[i].first-vec[j].first)+dpfunc(j,k-1));
        if(k==1) ans=min(ans,vec[i].second+vec[j].first+dpfunc(j,k-1));
    }
    return dp[i][k]=ans;
}
